...
class ParseSchemas {
    ...
    formatSchemaName(name) {
        return changeCase.pascalCase(name.replace(/^api\./i, ''));
    }
    /**
    ...
    parsePaths() {
        ...
        for (const url in paths) {
            ...
            for (const pathItemKey in pathItem) {
                ...
                const name = method + changeCase.pascalCase(path);
                const requestItems = tags[tag] || (tags[tag] = []);
                const paramsSchema = this.parseParamsSchema(operationItem);
                const item = {
                    name,
...
async function generate(outDir, docOptions, doc) {
    ...
    // 帕斯卡格式的作用域名称
    const scopeName = changeCase.pascalCase(docOptions.name);
    // 基础模版
    ...
    for (const tag in tags) {
        const folderName = changeCase.kebabCase(tag);
        const scopeTag = changeCase.pascalCase(tag);
        ...
        for (const requestItem of requestItems) {
            const scopeRefName = changeCase.pascalCase(requestItem.name);
        ...
        // 写入单个分组的类型声明文件
        prettier.format(dtsTemplate.replace(/:\s*\{/g, ': {\n'), {
            // semi: false,
            singleQuote: true,
            parser: 'babel-ts',
            printWidth: 9999,
        ...
        // 写入整个文档的类型声明文件
        writeFile(path.join(outDir, changeCase.kebabCase(docOptions.name + '-types') + '.d.ts'), (() => {
...
function buildApi(options) {
    ...
    for (const docOptions of options.docs) {
        ...
        const scopeOutDir = createDir(path.join(outDir, changeCase.kebabCase(name)));
        if (!link) {
